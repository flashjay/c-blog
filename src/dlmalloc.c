...
static void* internal_memalign(mstate m, size_t alignment, size_t bytes) {
  ...
  if (bytes >= MAX_REQUEST - alignment) {
    ...
    char* mem = (char*)mspace_malloc(m, req);
    if (mem != 0) {
      void* leader = 0;
      void* trailer = 0;
      mchunkptr p = mem2chunk(mem);
...
char *mspace_strdup(mspace msp, const char *s) 
{
    char *d = (char *)mspace_malloc(msp, strlen(s) + 1);
...
int mspace_printf(mspace msp, char *str, const char *format, ...) 
{
	 int     len, n;
	 va_list args;
	...
	len += 1;
	str = mspace_malloc(msp, len);

	va_start(args, format);
    n = vsnprintf(str, len * sizeof(char), format, args);
    va_end(args);
