...
// Declare proto. The user program must link against additional library  or include its own implementation of this function. 
...
#define M_INT_TO_STRING(buffer, buffer_len, val, signed) \
    do \
    { \
...
{
    tpl_node_t* node = (tpl_node_t*)m_malloc(len+1+sizeof(tpl_node_t));
    node->len = len;
...
{
    tpl_fcell_t* field = (tpl_fcell_t*)m_malloc(key_len + 1
                                                + sizeof(tpl_fcell_t));
    ...
    field->key[key_len] = 0;
    field->val = (char *)m_malloc(M_MAX(data_len, INITIAL_FIELD_LEN) + 1);
    field->len = 0;
    field->val[0] = 0;
    field->next = NULL;
    return field;
...
{
    tpl_tcell_t* section = (tpl_tcell_t*)m_malloc(key_len + 1
                                                  + sizeof(tpl_tcell_t)
                                                  + sizeof(tpl_t));
    ...
    while (*pfield != NULL)
    {
        if (memcmp((*pfield)->key, key, key_len) == 0)
        {
            if (data_len > INITIAL_FIELD_LEN && data_len > (*pfield)->len)
                (*pfield)->val = realloc((*pfield)->val, data_len);
...
static int tpl_construct(tpl_t *tpl, const char *p_last, const char *p_end)
{
                                ...
                                DELIMITER, DELIM_LEN)) != NULL)
    {
        ...
        {
            if ((p_curr - DELIM_LEN - p_last) != 0)
            {
                int val_len = p_curr - DELIM_LEN - p_last;
                *tail = create_node(val_len);
                (*tail)->val[val_len] = 0;
                (void)memcpy((*tail)->val, p_last, val_len);
                tail = &(*tail)->next;
...
{
    tpl_t *tpl = (tpl_t*)m_malloc(sizeof(tpl_t));
    tpl_init(tpl);
    return tpl;
}
...
void tpl_set_field(tpl_t* tpl, const char* key, const char* val, int len)
{
    ...
    if (field != NULL)
    {
        if (len > INITIAL_FIELD_LEN && len > field->len)
            field->val = m_realloc(field->val, len + 1);
...
void tpl_set_field_fmt(tpl_t* tpl, const char* key, const char* fmt, ...)
{
    ...
    if (field != NULL)
    {
        ...
        while (1)
        {
            ...
            field->len *= 2;
            field->val = m_realloc(field->val, field->len + 1);
        }
        field->len = n;
    }
}
...
void tpl_set_field_uint(tpl_t* tpl, const char* key, unsigned int val)
{
    ...
    if (field != NULL)
    {
        ...
        M_INT_TO_STRING(field->val, field->len, val, 0);
    } 
}
...
int tpl_save_as(const tpl_t* tpl, const char* filename)
{
    ...
    if (fp != NULL)
    {
        ...
        char *content = (char *)m_malloc(len + 1);

        tpl_get_content(tpl, content);

        if (fwrite(content, len, 1, fp) < (unsigned int)len)
        {
            (void)fclose(fp);
            m_free(content);
...
{  
    ...
    char *response = (char*)m_malloc(content_len + header_len + 1);
